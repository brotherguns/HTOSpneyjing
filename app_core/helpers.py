...
class ThreadButton(discord.ui.View):
    ...
    async def callback(self, _: discord.Button, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            await interaction.edit_original_response(content="Failed to create thread. Missing permissions or workspace error.")
        except discord.HTTPException as e:
            logger.error(f"Cannot create thread (HTTP): {e}")
            await interaction.edit_original_response(content="Failed to create thread. Please try again.")
